...
var fileInput  = document.querySelector("#my-file")
	fileInputPerfil  = document.querySelector("#my-file-perfil")
    the_return = document.querySelector("#contentFile");

if(fileInput) {
	fileInput.addEventListener( "change", function( event ) {
		the_return.innerHTML = '<img src="' + URL.createObjectURL(event.target.files[0]) + '">';
	});
}

if(fileInputPerfil) {
	fileInputPerfil.addEventListener( "change", function( event ) {
		the_return.innerHTML = '<img src="' + URL.createObjectURL(event.target.files[0]) + '" class="ms-auto me-auto rounded-circle img-fluid avatar-xxl">';
	});
}